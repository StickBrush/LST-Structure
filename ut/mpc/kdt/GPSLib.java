...
public class GPSLib {
	...
	public static double[] getSpaceBound(double[] lowk, double[] uppk){
		...
		if(Init.COORD_TYPE == Init.CoordType.GPS){
			tempRet = GPSLib.getCoordFromDist(lowk[0], lowk[1], Init.CoverageWindow.SPACE_RADIUS, 270);
			spaceBound[0] = tempRet[0];
			tempRet = GPSLib.getCoordFromDist(uppk[0], lowk[1], Init.CoverageWindow.SPACE_RADIUS, 90);
			spaceBound[1] = tempRet[0];
			tempRet = GPSLib.getCoordFromDist(uppk[0], lowk[1], Init.CoverageWindow.SPACE_RADIUS, 180);
			spaceBound[2] = tempRet[1];
			tempRet = GPSLib.getCoordFromDist(uppk[0], uppk[1], Init.CoverageWindow.SPACE_RADIUS, 0);
			spaceBound[3] = tempRet[1];
		} else if(Init.COORD_TYPE == Init.CoordType.Meters){
			spaceBound[0] = lowk[0] - Init.CoverageWindow.SPACE_RADIUS;
			spaceBound[1] = uppk[0] + Init.CoverageWindow.SPACE_RADIUS;
			spaceBound[2] = lowk[1] - Init.CoverageWindow.SPACE_RADIUS;
			spaceBound[3] = uppk[1] + Init.CoverageWindow.SPACE_RADIUS;		
		}
		return spaceBound;
	}
	...
	public static double[] getSpaceBoundQuick(double[] lowk, double[] uppk){
		...
		if(Init.COORD_TYPE == Init.CoordType.GPS){
			...
			double paddingY = .009 * Init.CoverageWindow.SPACE_RADIUS;
			double paddingX = .011 * Init.CoverageWindow.SPACE_RADIUS;
