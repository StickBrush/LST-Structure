...
import ut.mpc.kdplustree.KDPlusNode;
import ut.mpc.kdplustree.KDPlusTree;
import ut.mpc.setup.Init;

public class LSTTree extends KDPlusTree implements STStore {
	private boolean smartInsert = true;
	
	public LSTTree (int k) {
		super(k);
	}
	
	public LSTTree (int k, boolean smartInsert) {
		super(k);
	...
	public void windowQuerySweep(int optLevel){
		...
		corners = CoverageWindow.getBoundingBox(points);
		lowers[0] = corners[0];
		lowers[1] = corners[2];
		uppers[0] = corners[1];
		uppers[1] = corners[3];
	...
	public double windowQuery(boolean printWindow, int optLevel){
		...
		CoverageWindow wc = new CoverageWindow(lowers,uppers,points);
		if(optLevel == 1)
	...
	public double windowQuery(double[] lowk, double[] uppk, boolean printWindow, int optLevel){
		...
		CoverageWindow wc = new CoverageWindow(lowk,uppk,points);
		double returnVal;
		if(optLevel == 1)
	...
	public double CoverageOnPath(double[] startP, double[] endP, boolean chrono, boolean includeAdj){
		...
		double[] lowers = new double[2];
		double[] uppers = new double[2];
		corners = CoverageWindow.getBoundingBox(path);
		lowers[0] = corners[0];
		lowers[1] = corners[2];
		uppers[0] = corners[1];
		uppers[1] = corners[3];
		...
		if(includeAdj){ //Note, this does not include all points that could affect the bound
			...
			*/
			...
			wc = new CoverageWindow(lowers,uppers,points);
		} else {
			wc = new CoverageWindow(lowers,uppers,path);
		}
	...
	public List<Temporal> getTrajectory(boolean chrono){
		return this.findPath(0,Init.CoverageWindow.CURRENT_TIMESTAMP,true);
	}
	...
	public static void printBinaryTree(KDPlusNode root, int level){
	    if(root==null)
	         return;
	...
	public static double[] getBalanceRecurse(KDPlusNode root){
	...
	public void printRecurse(KDPlusNode node){
		if(node != null){
